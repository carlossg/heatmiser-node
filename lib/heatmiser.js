...
Heatmiser.prototype.command = function(operation, success, error, data) {
  len = 7 + data.length;
  var buf = new Buffer([operation, w2b(len)[0], w2b(len)[1], w2b(this.pin)[0], w2b(this.pin)[1]]);
  buf = Buffer.concat([buf, (new Buffer(data))]);
  var crc = crc16(buf)
  buf = Buffer.concat([buf, (new Buffer(w2b(crc)))]);
...
Heatmiser.prototype.read_device = function(success, error){
  this.command(0x93, success, error, [0x00, 0x00, 0xFF, 0xFF]);
}
